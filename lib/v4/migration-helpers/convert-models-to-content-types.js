...
const convertModelToContentType = async (apiPath, contentTypeName) => {
  ...
  try {
    ...
    let _singularName = _.kebabCase(pluralize.singular(contentTypeName));
    let _pluralName = _.kebabCase(pluralize(contentTypeName));
    if (
      pluralize.isPlural(contentTypeName) &&
      String(process.env.SMT_KEEP_PLURAL).toUpperCase() == "TRUE"
    ) {
      _singularName = _.kebabCase(pluralize(contentTypeName));
      _pluralName = _.kebabCase(pluralize.singular(contentTypeName));
    }
    const infoUpdate = {
